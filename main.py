...
from config import TOKEN  # Убедись, что этот файл у тебя есть!
...
async def registration(message: Message):
    ...
    try:
        ...
        if user:
            ...
            logging.info(f"Новый пользователь зарегистрирован: {telegram_id}, {name}")

    except sqlite3.Error as e:
        logging.error(f"Ошибка SQLite при регистрации: {e}")
...
async def exchange_rates(message: Message):
    url = "https://v6.exchangerate-api.com/v6/bddec7e0ec531b0967ba7047/latest/USD"
    try:
...
# Личные финансы (начало заполнения)
@dp.message(F.text == "Личные финансы")
async def finances(message: Message, state: FSMContext):
    await state.set_state(FinancesForm.category1)
    await message.reply("Введите первую категорию расходов:")

# Обработчики состояний (финансовые категории и расходы)
...
async def finances_expenses3(message: Message, state: FSMContext):
    ...
    try:
        ...
        cursor.execute('''UPDATE users SET category1 = ?, expenses1 = ?, 
                          category2 = ?, expenses2 = ?, 
        ...
        logging.error(f"Ошибка SQLite при записи расходов: {e}")
